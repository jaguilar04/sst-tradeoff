...
import torch
from fixed_segmenter.baseline_iwslt.speech_segmenter import SpeechSegmenter, TranslateAction
...
class SeamlessM4TAgent(SpeechToTextAgent):
    ...
    def __init__(self, args, segmenter: SpeechSegmenter):
        ...
        self.step_length = float(_get_arg(args, "step_length", 1.0))
        ...
        self.tgt_lang = _get_arg(args, "seamless_m4t_tgt_lang", "deu")
        self.device = _get_arg(
            args,
            "device",
            "cuda" if torch.cuda.is_available() else "cpu",
        )
        self.seamless_m4t_model = self.seamless_m4t_model.to(self.device)
        self.output_words_valid_words_only = _get_arg(
            args,
            "eval_latency_unit",
            getattr(args, "latency_unit", "word"),
        ) == "word"
    ...
    def _translate(self, states: AgentStates, segmenter_action: TranslateAction):
        ...
        audio_inputs = self.seamless_m4t_processor(
            audio=speech_to_translate, return_tensors="pt")
