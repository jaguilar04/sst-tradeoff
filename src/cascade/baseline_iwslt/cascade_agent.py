...
from cascade.baseline_iwslt.speech_segmentation import SpeechSegmenter, TranslateAction
...
class CascadeAgent(SpeechToTextAgent):
    ...
    def __init__(self, args, segmenter : SpeechSegmenter):
        ...
        whisper_compute_type = getattr(args, "whisper_compute_type", "auto")
        if whisper_compute_type == "auto":
            whisper_compute_type = "float16" if self.device == "cuda" else "float32"
        try:
            ...
            # Some GPUs/backends cannot run float16 efficiently; fallback to float32.
            if whisper_compute_type == "float16":
                logging.warning(
                    "Whisper float16 is not supported efficiently on this backend. "
