#!/bin/bash
#SBATCH --job-name=cascade_2026_partial_mbr_grid
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/mbr_trades/cascade_2026_partial_mbr/slurm-%j.out
...
EXP_ROOT="${REPO_ROOT}/experiments/mbr_trades/cascade_2026_partial_mbr"
...
# Partial MBR: applies epsilon-sampling + MBR ONLY at utterance close.
# During the local agreement region, greedy decoding is used (temp=0, 1 sample).
PROCESSOR_TYPE="cascade_2026.agent_simulstream_partial_mbr.CascadeSpeechProcessor"
...
echo "Partial MBR grid (utterance close only): seg=${SEGMENT_SIZE}s, epsilon=${EPSILON}" > "${SUMMARY_FILE}"
...
# ── Launch info ───────────────────────────────────────────────────────────────
echo "════════════════════════════════════════════════════════════"
echo "  Partial MBR grid: segment_size=${SEGMENT_SIZE}s (fixed), epsilon=${EPSILON}"
...
for method in "${DECODING_METHODS[@]}"; do
    for n in "${N_VALUES[@]}"; do
        ...
        run_name="partialmbr_${method}_n${n}_${SEG_TAG}"
...
# NOTE: temperature/epsilon/n_samples only apply at utterance close.
# In the local agreement region (utt_finished=False) greedy is used (temp=0, 1 sample).
...
# MBR decoding method (applied at utterance close only)
