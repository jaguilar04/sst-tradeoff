#!/bin/bash
#SBATCH --job-name=cascade_2026_epsilon_eval
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/mbr_trades/cascade_2026_epsilon/slurm-eval-%j.out
...
# ── Paths ─────────────────────────────────────────────────────────────────────
EXP_DIR="${REPO_ROOT}/experiments/mbr_trades/cascade_2026_epsilon"
...
CSV_OUT="${EXP_DIR}/results_cascade_2026_epsilon.csv"
...
row_is_complete() {
    local name="$1"
    local row
    row=$(grep "^${name}," "${CSV_OUT}" 2>/dev/null || true)
    [[ -z "${row}" ]] && return 1
...
remove_row() {
    ...
    local tmp
    tmp=$(mktemp)
    grep -v "^${name}," "${CSV_OUT}" > "${tmp}" || true
    mv "${tmp}" "${CSV_OUT}"
}

# ── Loop over all JSONL files ─────────────────────────────────────────────────
for jsonl_file in "${INFERENCES_DIR}"/cascade_2026_epsilon_*.jsonl; do
    ...
    # Skip if already evaluated with no NA values
    if row_is_complete "${basename}"; then
        echo "[eval] Already complete: ${basename}, skipping..."
        continue
    fi

    # Remove incomplete row to rewrite it cleanly
    remove_row "${basename}"
