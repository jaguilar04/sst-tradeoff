#!/bin/bash
#SBATCH --job-name=cascade_2026_epsilon_infer
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/mbr_trades/cascade_2026_epsilon/slurm-%j.out
...
for seg in "${SEGMENT_SIZES[@]}"; do
    ...
    run_name="cascade_2026_epsilon_seg${seg_ms}"
...
type: "cascade_2026.agent_simulstream_epsilon.CascadeSpeechProcessor"
...
CONFIG
    ...
    if [[ -f "${metrics_file}" ]]; then
        echo "[cascade_2026_epsilon] $(basename ${metrics_file}) already exists, skipping..."
        continue
    fi

    echo "════════════════════════════════════════════════════════════"
    echo "[cascade_2026_epsilon] segment_size=${seg}s (${seg_ms}ms)"
    echo "════════════════════════════════════════════════════════════"
    ...
    echo "[cascade_2026_epsilon] Done segment_size=${seg}s"
    echo "────────────────────────────────────────────────────────────"
