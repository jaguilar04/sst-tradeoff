#!/bin/bash
#SBATCH --job-name=policies_asr_grid
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/policies_trades/asr/slurm-%j.out
...
# then set the node below (or comment it out to let SLURM pick any node).
#SBATCH --nodelist=vrhpc6.dsic.upv.es

# ASR EMISSION POLICY GRID on cascade_2026 with partial MBR
# (MBR applied only at utterance close), fixed segment size of 0.96 s.
#
# Policies (asr_policy key):
#   - hold_n            : static baseline; waits for n chunks before emitting.
#   - local_agreement   : exact common prefix (current default).
#   - tolerant_agreement: LACP; tolerates tau Levenshtein edits per word.
#
# Each policy is tested with all 3 decoders from the paper.
...
EXP_ROOT="${REPO_ROOT}/experiments/policies_trades/asr"

# ── Policy grid ───────────────────────────────────────────────────────────────
# Format: "policy|param"  (param = n for hold_n, tau for tolerant_agreement)
RUNS=(
    "hold_n|1"
    "hold_n|2"
    "hold_n|4"
    ...
    "tolerant_agreement|3"
)

# To run a subset in parallel: POLICY_FILTER = space-separated list of policies.
# Empty = run all. Used by launch_grid_parallel.sh to distribute the grid across nodes.
...
PROCESSOR_TYPE="cascade_2026.agent_simulstream_asr_policies.CascadeSpeechProcessor"

# ── Fixed parameters ──────────────────────────────────────────────────────────
ASR_MODEL="Qwen/Qwen3-ASR-1.7B"
LLM_MODEL="Qwen/Qwen3-4B-Instruct-2507"
# GPU A30 = 24 GB. ASR requires >=0.36 (less causes negative KV cache);
# set LLM to 0.36 to leave headroom for the forced aligner/cuBLAS (avoids CUBLAS crash).
ASR_GPU_MEM="0.36"
LLM_GPU_MEM="0.36"
SOURCE_LANG="English"
TARGET_LANG="German"
...
echo "ASR policy grid: seg=${SEGMENT_SIZE}s, partial-MBR, epsilon=${EPSILON}" > "${SUMMARY_FILE}"
...
echo "════════════════════════════════════════════════════════════"
echo "  ASR policy grid: segment_size=${SEGMENT_SIZE}s, partial MBR"
...
for run in "${RUNS[@]}"; do
    ...
    # Policy tag and per-policy values (irrelevant fields remain at defaults).
    HOLD_N_CHUNKS=2
    TOLERANT_TAU=2
    case "${policy}" in
        hold_n)
            HOLD_N_CHUNKS="${param}"
            policy_tag="hold_n${param}"
            ;;
        tolerant_agreement)
            TOLERANT_TAU="${param}"
            policy_tag="tolerant_tau${param}"
            ;;
        local_agreement)
            policy_tag="local_agreement"
            ;;
        *)
    ...
    for method in "${DECODING_METHODS[@]}"; do
        for n in "${N_VALUES[@]}"; do
            ...
            run_name="asr_${policy_tag}_${method}_n${n}_${SEG_TAG}"
...
# ASR emission policy
asr_policy: "${policy}"
hold_n_chunks: ${HOLD_N_CHUNKS}
tolerant_tau: ${TOLERANT_TAU}
