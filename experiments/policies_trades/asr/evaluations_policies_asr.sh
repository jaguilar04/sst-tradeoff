#!/bin/bash
#SBATCH --job-name=policies_asr_eval
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/policies_trades/asr/slurm-eval-%j.out

# Evaluation of the ASR policy grid: latency (LongYAAL), MT quality
...
CSV_OUT="${EXP_ROOT}/results_policies_asr.csv"

# ── Grid (must match inferences_policies_asr.sh) ──────────────────────────────
RUNS=(
    "hold_n|1"
    "hold_n|2"
    "hold_n|4"
...
if [[ ! -f "${CSV_OUT}" ]]; then
    echo "policy_tag,policy,param,method,n_samples,run_name,long_yaal_nca,long_yaal_ca,bleu,chrf_pp,comet_xl,wer" \
        > "${CSV_OUT}"
fi
...
for run in "${RUNS[@]}"; do
    ...
    param="${run##*|}"
    case "${policy}" in
        hold_n)             policy_tag="hold_n${param}" ;;
        tolerant_agreement) policy_tag="tolerant_tau${param}" ;;
        local_agreement)    policy_tag="local_agreement" ;;
    ...
    for method in "${DECODING_METHODS[@]}"; do
        for n in "${N_VALUES[@]}"; do
            ...
            if [[ -f "${transcript_file}" ]]; then
                wer_out=$(python3 "${EXP_ROOT}/compute_wer.py" \
            ...
            echo "${policy_tag},${policy},${param},${method},${n},${run_name},${long_yaal_nca},${long_yaal_ca},${bleu},${chrf_pp},${comet_xl},${wer}" \
                >> "${CSV_OUT}"
