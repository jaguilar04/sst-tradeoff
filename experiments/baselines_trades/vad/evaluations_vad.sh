#!/bin/bash
#SBATCH --job-name=eval_vad
#SBATCH --partition=batch
#SBATCH --gpus=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=32G
#SBATCH --output=/data/jagucam1@alumno.upv.es/MAIN/experiments/baselines_trades/vad/slurm-eval-%j.out
...
# ── Paths ─────────────────────────────────────────────────────────────────────
BASELINE="vad"
EXP_DIR="${REPO_ROOT}/experiments/baselines_trades/${BASELINE}"
...
CSV_OUT="${EXP_DIR}/results_vad.csv"
...
for jsonl_file in "${INFERENCES_DIR}"/${BASELINE}_*.jsonl; do

    basename=$(basename "${jsonl_file}" .jsonl)
    config_file="${CONFIGS_DIR}/${basename}.yaml"

    # Skip if already in the CSV
    if grep -q "^${basename}," "${CSV_OUT}" 2>/dev/null; then
        echo "[eval] Already in CSV: ${basename}, skipping..."
        continue
    fi
